// import { screenResizeHandler } from './screenHandler';
...
function renderControl(ctx, left, top) {
  ctx.save();
...
// mtr 控件自定义渲染
function renderMtrControl(ctx, left, top) {
  ctx.save();
  ctx.translate(left, top);
...
function renderActiveSelectionControl(ctx, left, top) {
  ctx.save();
  ctx.translate(left, top);
...
function getImageControls() {
  const {
    ...
    scaleCursorStyleHandler: scaleStyleHandler,
    scalingEqually,
    scalingYOrSkewingX,
    scalingXOrSkewingY,
...
function getCustomBackgroundControls() {
  const { ml, mr, mb, mt, tl, tr, bl, br } = getImageControls();
  return { ml, mr, mb, mt, tl, tr, bl, br };
}
...
function getScreenControls() {
  ...
  const resizeHandler = wrapWithFireEvent(
    'resizing',
    wrapWithFixedAnchor(screenResizeHandler)
  );

  const ml = new fabric.Control({
    x: -0.5,
    y: 0,
    actionHandler: resizeHandler,
    cursorStyleHandler: scaleSkewStyleHandler,
    actionName: 'resizing',
    render: renderControl,
    offsetX: -1,
  ...
  return { ml, mr, mt, mb, tl, tr, bl, br };
}


function getCustomActiveSelectionControls() {
...
export const backgroundControls = getCustomBackgroundControls();
export const textControls = getTextControls();
export const screenControls = getScreenControls();
export const activeSelectionControls = getCustomActiveSelectionControls();
